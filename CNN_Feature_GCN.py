# This code use CNN derived ECG and PPG features to train GCN model
...
#
    F = np.array(F)
    L = np.array(L)
    np.save("D:/MLDS/features/overall.npy", F)
    np.save("D:/MLDS/labels/overall.npy", L)
    return F, L
...
class GCNmodel(nn.Module):
    def __init__(self, input_ch = 64, hidden_ch =192, num_classes=9):
        super().__init__()
...
def fully_connected_edge(num_classes=9):
...
def feature_to_graph(F, label, edge_index, num_classes=9):
    data = []

    for n in range(len(F)):
        x = torch.from_numpy(F[n].astype(np.float32))
...
if __name__ == "__main__":
    ...
    label = np.load("D:/MLDS/labels/cnn_overall.npy")
    ...
    edge_index = fully_connected_edge(num_classes=9)
    ...
    model = GCNmodel(input_ch=64, hidden_ch=64, num_classes=9).to(DEVICE)

    cls_counts = np.bincount(y_train.astype(int), minlength=9)
    cls_weights = 1.0 / (cls_counts + 1e-6)
    cls_weights = cls_weights * (9 / cls_weights.sum())
    ...
    epochs = 20 # if it is too small, then increase to 20
    best_val_acc = -1
    best_state = None
